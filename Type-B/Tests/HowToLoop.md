...
| **1.4** | `p1_4_UnwindAndVerifyProfit()` | Closes the loop position (Repay All, Withdraw All). Verification: Final profit is checked to be $\mathbf{\geq 1.5}$ ETH after fees and slippage. |
...
| Step | Function | Key Action & Verification |
| :--- | :--- | :--- |
| **2.1** | `p2_1_PrepareFunds()` | Mints **$20,000 USDT** (Token6) margin and resets prices to WETH = $2000, USDT = $1$. |
| **2.2** | `p2_2_Execute10xShort()` | Executes the leveraged loop with a **10.0x target**. The position is highly volatile (e.g., $200k collateral with $180k debt). |
| **2.3** | `p2_3_VerifyRisk()` | Checks the resulting risk profile. Verification: Health Factor must be **low (e.g., HF < 1.25)** to confirm high-leverage risk, but must be above the liquidation threshold **(HF > 0.99)**. |## Running Leverage Loop Strategy Tests in Remix (Updated Guide)
